...
- In terms of .NET desktop development, Sunny’s strategy is to make full use of open source technology instead of competing with browsers, and build the desktop software ecosystem directly on the basis of the browser framework while retaining the advantages of desktop software. This idea is The original vision of VBR (Virtual Web Browser Engine).
...
- 在.NET 桌面开发方面，Sunny的策略是与其与浏览器竞争，不如充分利用开源技术，将桌面软件生态构建在保留桌面软件优势的前提下直接建立在浏览器框架基础之上，这一构想就是VBR的最初愿景。
